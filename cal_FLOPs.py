...
def asfsr(h = 720, w = 1280, flops = True, scale = 2, hf_ratio=0.0, mf1_ratio=0.0, mf2_ratio=0.0, mf3_ratio=0.0, lf_ratio=0.0, r = [4]):
    avg = 0
    ...
    high = sum([hpf(5, lr, 1, 32),
               ...
               hpf(1, lr, 16, 32),
               hpf(9, hr, 32, 1)]) * (hf_ratio)

    mid1 = sum([lpf(5, lr, 1, 32, r[0]),
               lpf(1, lr, 32, 16, r[0]),
               lpf(3, lr, 16, 16, r[0])*4,
               lpf(1, lr, 16, 32, r[0]),
               lpf_last_conv(9, hr, 32, 1, r[0])]) * (mf1_ratio)

    mid2 = sum([lpf(5, lr, 1, 32, r[1]),
               lpf(1, lr, 32, 16, r[1]),
               lpf(3, lr, 16, 16, r[1])*4,
               lpf(1, lr, 16, 32, r[1]),
               lpf_last_conv(9, hr, 32, 1, r[1])]) * (mf2_ratio)

    mid3 = sum([lpf(5, lr, 1, 32, r[2]),
               lpf(1, lr, 32, 16, r[2]),
               lpf(3, lr, 16, 16, r[2])*4,
               lpf(1, lr, 16, 32, r[2]),
               lpf_last_conv(9, hr, 32, 1, r[2])]) * (mf3_ratio)

    low = sum([lpf(5, lr, 1, 32, r[2]),
               lpf(1, lr, 32, 16, r[2]),
               lpf(3, lr, 16, 16, r[2])*4,
               lpf(1, lr, 16, 32, r[2]),
               lpf_last_conv(9, hr, 32, 1, r[2])]) * (lf_ratio)

    # this part has no skipping
    # lpf에서 한번에 계산햇던걸 나누신 듯
    comn=0
    if lf_ratio !=0:
        comn = sum([lpf_last_reduce(lr, 32, r[0]),
                    lpf_last_reduce(lr, 32, r[1]),
                    lpf_last_reduce(lr, 32, r[2]),
                    lpf_last_reduce(lr, 32, r[3]), # before transposed convolution
                    lr]) # mask generation : FLOPs due to averaging (division)

    result = comn + high + mid1+ low + mid2 + mid3
...
# spars = [0, 0.45]
spars = [0.22, 0.1, 0.14, 0.2, 0.34]
r = [2, 4, 8, 16]
for spar in spars:
    asfsr(hf_ratio=spars[0], mf1_ratio=spars[1], mf2_ratio=spars[2],  lf_ratio=spars[3], scale = 2, r = r) # mf3_ratio=spars[3],
# for spar in spars:
    # asfsr(hf_ratio=spars[0], mf1_ratio=spars[1], mf2_ratio=spars[2], lf_ratio=spars[3], scale = 2, r = r)
#
# asfsr(spar=0)
