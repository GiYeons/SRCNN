...
class Tconv_block(nn.Module):
    def __init__(self, scale, in_c, out_c, ker, r):
        ...
        self.mid1_par2 = nn.ConvTranspose2d(
            in_channels=in_c//r[0], out_channels=out_c, kernel_size=ker, padding=ker//2, stride=scale, output_padding=scale-1)

        self.mid2_par1 = nn.Conv2d(
            in_channels=in_c, out_channels=in_c//r[1], kernel_size=1)
        self.mid2_par2 = nn.ConvTranspose2d(
            in_channels=in_c//r[1], out_channels=out_c, kernel_size=ker, padding=ker//2, stride=scale, output_padding=scale-1)

        self.low_par1 = nn.Conv2d(
            in_channels=in_c, out_channels=in_c//r[2], kernel_size=1)
        self.low_par2 = nn.ConvTranspose2d(
            in_channels=in_c//r[2], out_channels=out_c, kernel_size=ker, padding=ker//2, stride=scale, output_padding=scale-1)
    ...
    def forward(self, x, mask_high, mask_mid1, mask_mid2, mask_low, eval=False):
        # if eval==True:
...
class Conv_block(nn.Module):
    def __init__(self, in_c, out_c, ker, r):
        ...
        self.low_par1 = nn.Conv2d(in_channels=in_c, out_channels=out_c // r[2], kernel_size=ker, padding=ker // 2)
        self.low_par2 = nn.Conv2d(in_channels=out_c // r[2], out_channels=out_c, kernel_size=1)
...
class Net(nn.Module):
    ...
    def create_mask(self, x, th, dilker, dilation=True):
        ...
        return mask_high, mask_mid1, mask_mid2, mask_low
    ...
    def forward(self, x, th=[0.04], dilker=3, dilation=True, eval=False):
        ...
        mask_high, mask_mid1, mask_mid2, mask_low = self.create_mask(x, th, dilker, dilation)

        orix = x

        x = self.relu(self.first_part(x, mask_high, mask_mid1, mask_mid2, mask_low, eval=False))
        x = self.relu(self.reduction(x, mask_high, mask_mid1, mask_mid2, mask_low, eval=False))

        x = self.relu(self.mid_part1(x, mask_high, mask_mid1, mask_mid2, mask_low, eval=False))
        x = self.relu(self.mid_part2(x, mask_high, mask_mid1, mask_mid2, mask_low, eval=False))
        x = self.relu(self.mid_part3(x, mask_high, mask_mid1, mask_mid2, mask_low, eval=False))
        x = self.relu(self.mid_part4(x, mask_high, mask_mid1, mask_mid2, mask_low, eval=False))

        x = self.relu(self.expansion(x, mask_high, mask_mid1, mask_mid2, mask_low, eval=False))
        ...
        y = self.last_part(x, mask_high, mask_mid1, mask_mid2, mask_low, eval=False)
