...
class Tconv_block(nn.Module):
    ...
    def forward(self, x, mask_high, mask_mid1, mask_low, eval=False):
        # if eval==True:
...
class Conv_block(nn.Module):
    def __init__(self, in_c, out_c, ker, r):
        ...
        self.low_par1 = nn.Conv2d(in_channels=in_c, out_channels=out_c // r[1], kernel_size=ker, padding=ker // 2)
        self.low_par2 = nn.Conv2d(in_channels=out_c // r[1], out_channels=out_c, kernel_size=1)
...
class Net(nn.Module):
    ...
    def create_mask(self, x, th, dilker, dilation=True):
        ...
        mask_low = torch.where(mask_mid1==0, 1, 0)
        mask_mid1 = mask_mid1 - mask_high

        return mask_high, mask_mid1, mask_low
    ...
    def forward(self, x, th=[0.04], dilker=3, dilation=True, eval=False):
        ...
        mask_high, mask_mid1, mask_low = self.create_mask(x, th, dilker, dilation)

        orix = x

        x = self.relu(self.first_part(x, mask_high, mask_mid1, mask_low, eval=False))
        x = self.relu(self.reduction(x, mask_high, mask_mid1, mask_low, eval=False))

        x = self.relu(self.mid_part1(x, mask_high, mask_mid1, mask_low, eval=False))
        x = self.relu(self.mid_part2(x, mask_high, mask_mid1, mask_low, eval=False))
        x = self.relu(self.mid_part3(x, mask_high, mask_mid1, mask_low, eval=False))
        x = self.relu(self.mid_part4(x, mask_high, mask_mid1, mask_low, eval=False))

        x = self.relu(self.expansion(x, mask_high, mask_mid1, mask_low, eval=False))

        mask_high = self.upsample_mask(mask_high)
        mask_mid1 = self.upsample_mask(mask_mid1)
        mask_low = self.upsample_mask(mask_low)

        y = self.last_part(x, mask_high, mask_mid1, mask_low, eval=False)
